...
arrWorkstations = Array("XP","Vista","Windows 7","Windows 8", "Windows 8.1")
					...
					If InStr(1, strDescription, "]", 1) > 0 Then
						...
						If DateDiff("d", dtMovedDate, Now ) > intMaxDaysInQuarantine Then
							...
							If intReturnDisable = 0 Then
								If Instr(1,objRecordSet.Fields("operatingSystem").value,"Server",1) > 0 Then
									GblstrServerEmailMessage = GblstrServerEmailMessage & "Disabled computer account "&objRecordset.Fields("cn").value& ".  Account was moved on "& dtMovedDate &"."&vbCrLf
								Else
									GblstrWorkstationEmailMessage = GblstrWorkstationEmailMessage & "Disabled computer account "&objRecordset.Fields("cn").value& ".  Account was moved on "& dtMovedDate &"."&vbCrLf
								End If
								'move account
								intReturnMove = 0
		...
		If rsData.Fields.Item("DaysOld").value > intMaxDaysOldQurantine Then
			If InStr(1, rsData.Fields.Item("DN").value, GblstrComputerQuarantineActive&","&strDefaultNamingContext, 1) = 0 Then
				...
				If intReturnMove = 0 Then
					...
					Else
						GblstrWorkstationEmailMessage = GblstrWorkstationEmailMessage & "Moved "&rsData.Fields.Item("Computer").value&" with last logon of "&rsData.Fields.Item("LastLogon").value&" from "&rsData.Fields.Item("DN").value&" to "&GblstrComputerQuarantineActive&","&strDefaultNamingContext&"."& vbCrLf
					End If
					intReturnDesc = 0
					If blnTestOnly = False Then
...
Sub GetComputerObjectInfo()
	...
	objCommand.Properties("Page Size") = 99999
	objCommand.Properties("Chase referrals") = &H60
	objCommand.CommandText = "<LDAP://"&strDefaultNamingContext&">;(objectCategory=computer);distinguishedName,whenCreated,lastLogonTimeStamp,cn,operatingSystem;subtree"
	Set objRecordSet = objCommand.Execute
	Do Until objRecordSet.EOF
		blnGo = True
